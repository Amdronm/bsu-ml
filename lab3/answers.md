...
ROC-AUC — это вероятность того, что если мы возьмем случайно выбранного "уходящего" клиента (1) и случайно выбранного "лояльного" (0), то модель даст "уходящему" более высокую оценку риска, чем "лояльному".

Если AUC = 0.85, значит в 85% случаев ваша модель правильно отранжирует эту пару (уходящий будет выше в списке риска, чем лояльный).

В оставшихся 15% случаев модель ошибется и скажет, что лояльный клиент рискует больше.



LabelEncoder: Предназначен только для целевой переменной (yy). Он берет массив ['Cat', 'Dog', 'Cat'] и делает [0, 1, 0]. Работает с 1D массивом.

OrdinalEncoder: Предназначен для матрицы признаков (XX). Он может переварить таблицу с кучей колонок сразу. Работает с 2D массивом (n_samples, n_features).